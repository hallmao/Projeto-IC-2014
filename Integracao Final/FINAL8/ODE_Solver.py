...
####Handlers-------------------------------------------
  #user input -># ao,a1,a2,y0,dy0,xT
    def a0_handler(self,event):
        ...
        self.in_a0.configure(validate = 'focus',validatecommand = (okay_entry_a0,'%s','a0'))
        a0 = self.in_a0.get()
    ...
    def a1_handler(self,event):
        ...
        self.in_a1.configure(validate = 'focus',validatecommand = (okay_entry_a1,'%s','a1'))
        a1 = self.in_a1.get()
        print "User input",user_input
        print "In a1 now",a1
        if user_input[1] != a1:
    ...
    def a2_handler(self,event):
        ...
        self.in_a2.configure(validate = 'focus',validatecommand = (okay_entry_a2,'%s','a2'))
        a2 = self.in_a2.get()
        print "User input",user_input
        print "In a2 now",a2
        if user_input[2] != a2:
    ...
    def inxT_handler(self,event):
        ...
        self.in_xT.configure(validate = 'focus',validatecommand = (okay_entry_xT,'%s','xT'))
        xT = self.in_xT.get()
        print "User input",user_input
        print "In xT now",xT
        if user_input[5] != xT:
 ...
 #user input -># ao,a1,a2,y0,dy0,xT
    def y0_handler(self,event):
        ...
        self.in_y0.configure(validate = 'focus',validatecommand = (okay_entry_y0,'%s','y0'))
        y0 = self.in_y0.get()
        print "User input",user_input
        print "In y0 now",y0
        if user_input[3] != y0:
    ...
    def dy0_handler(self,event):
      #print "apertei",self.in_a2.get()
        ...
        self.in_dy0.configure(validate = 'focus',validatecommand = (okay_entry_dy0,'%s','dy0'))
        dy0 = self.in_dy0.get()
        print "User input",user_input
    ...
    def set_idiomas(self):
        ...
        if (idi.get() == 1): #Port
            self.menubar.entryconfig(1,label="Idioma")
            ...
            self.menubar.entryconfig(1,label="Language")
            ...
            self.cond_ini_label.configure(text='''Initial Conditions      y(t) =                         y'(t) =''')
            ...
            self.TNotebook1.tab(1,text="Graficos")
            self.TNotebook1.tab(2,text="Log Texto")
            self.cond_ini_label.configure(text='''Condiciones Iniciales   y(t) =                         y'(t) =''')
            self.xT_label.configure(text='''Entrada x(t) =''')
            set_lingua(3)
            self.update_saida()
            #print "espanol"
    ...
    def __init__(self, master):
        ...
        self.in_dy0.place(relx=0.41,rely=0.12,relheight=0.04,relwidth=0.04)
