...
def print_latex():
        ...
        #plt.clf()
        #edo_main()
        for i in range(1,7):Respostas[i] = Respostas[i].evalf(prec)

        # 0- Raizes; 1- Forma Natural da respsota; 2- Resposta Natural;
        # 3- Resposta Particular; 4- Resposta Transitoria; 5- Respsota Forcada
        # 6- Resposta Completa; 7-Sinal de entrada x(t); 8 - eq
        ###
        eqD = Respostas[8].subs(Respostas[7],0)
        ...
        plt.title('')
        plt.text(xdif,0.89,idi_eq+ur'$'+eqDiferencialEntradaLatex+'$'+"  "+ur'$'+latex("=")+'$'+"  "+ur''+RespostasEmLatex[7])
        ...
        if((const[5] == 0) and (const[4] == 0) and (const[3] == 0) and (const[2] == 0)):#eq ordem 1
            plt.text(xdif,0.9-3*dif,ur''+idi_yn+ur'$'+latex("(")+'$'+ur''+RespostasEmLatex[2]+ur'$'+latex(")u(t)")+'$'+"    "+idi_cond_sing+ur'$'+latex("\ y(0)=\ ")+'$'+ur'$'+str(latex(cond_ini[0]))+'$')
        else:
            plt.text(xdif,0.9-3*dif,ur''+idi_yn+ur'$'+latex("(")+'$'+ur''+RespostasEmLatex[2]+ur'$'+latex(")u(t)")+'$'+"    "+idi_cond_pl+ur'$'+latex("\ y(0)=\ ")+'$'+ur'$'+str(latex(cond_ini[0]))+'$'+ur'$'+latex("\quad dy(0)=\ ")+'$'+ur'$'+str(latex(cond_ini[1]))+'$')
        ...
        plt.text(xdif,0.9-5*dif,ur''+idi_yt+ur'$'+latex("(")+'$'+ur''+RespostasEmLatex[4]+ur'$'+latex(")u(t)")+'$')
        plt.text(xdif,0.9-6*dif,ur''+idi_yf+ur'$'+latex("(")+'$'+ur''+RespostasEmLatex[5]+ur'$'+latex(")u(t)")+'$')
        plt.text(xdif,0.9-7*dif,ur''+idi_yc+ur'$'+latex("(")+'$'+ur''+RespostasEmLatex[6]+ur'$'+latex(")u(t)")+'$')
        #plt.text(xdif,0.9-7*dif,'x(t) = '+ur''+RespostasEmLatex[7])
        

        plt.subplots_adjust(left=0.12, bottom=0.08, right=0.50, top=0.88,
