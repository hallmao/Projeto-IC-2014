...
def show_plots():
       ...
       # plt_r.set_ylim(-abs(abs(float(plotRaizesC[0])) +1),abs(abs(float(plotRaizesC[len(plotRaizesC)-1]))+ 1)  )
        #plt_r.set_xlim(-abs(abs(float(plotRaizesR[0]))+1),abs(abs(float(plotRaizesR[len(plotRaizesR)-1]))+1)  )
        if plotRaizesC[0] != 0:
...
##                                         print "Entrei"+ " ValoremT ",valorEmT,"Fn:",fN,type(valorEmT),type(fN)
                ...
                for i in range(1,7): #arruma precisao
                        ...
                        Respostas[i] = nsimplify(Respostas[i], rational = True,tolerance = 1e-3).evalf(prec)
        except:
                pass
