...
def set_xT(expr):
        try:
                Respsotas[7] = eval(expr)
        except:
                pass
        
        try:
                Respostas[7] = float(expr)
        except:
                pass
...
def init(a2, a1, a0, xT, y0, dy0):
        ...
        eq = sympify(
                 const[5] * y(t).diff(t, 5) + const[4] * y(t).diff(t, 4) + const[3] * y(t).diff(t, 3) + const[2] * y(t).diff(t, 2) + const[1] * y(t).diff(
                                t) + const[0] * y(t) - xT)
        Respostas[8] = eq
...
def log_print():
        ...
        equacao = str(equacao)
        equacao = "Equacao: "+equacao+" = "+str(Respostas[7])
        #print equacao
