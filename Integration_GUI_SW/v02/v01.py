...
def log_print():
    ...
    if b0 != 0:
        ...
        print "yfor(t) = " + str(round(-b0 / a0, precLog)) + "*(" + xT + ")*e^(" + str(
            round(r, precLog)) + "t)*u(t) + " + str(round(b0 / a0, 2)) + "*" + xT + "*u(t)"
        ###Resposta completa
        print "\nResposta completa : yc(t) = ynat(t) + yfor(t)"
        print "yc(t) = (" + str(round(y0, precLog)) + str(round(-b0 / a0, precLog)) + "*(" + xT + ")*e^(" + str(
            round(r, precLog)) + "t)*u(t) + " + str(round(b0 / a0, precLog)) + "*" + xT + "*u(t)"
...
def plot_and_show():
    ...
    outputAllPlots = plt.figure("EDOs a coeficientes constantes")
    plt.subplot(333)
    ...
    ##Manual Plot spacing
    plt.subplots_adjust(left=0.07, bottom=0.12, right=0.98, top=0.91,
                        wspace=0.34, hspace=0.80)

    return outputAllPlots
...
def vp_start_gui():
    ...
    root.geometry('800x600+67+74')
    v01_support.set_Tk_var()
    w = EDO_Solver(root)
    v01_support.init(root, w)
    root.mainloop()
...
def create_EDO_Solver(root, param=None):
    ...
    w = Toplevel(root)
    w.title('EDO_Solver')
    w.geometry('800x600+67+74')
    v01_support.set_Tk_var()
    w_win = EDO_Solver(w)
    v01_support.init(w, w_win, param)
    return w_win
...
class EDO_Solver:
    def __init__(self, master=None):
        ...
        self.Relat.configure(variable=v01_support.output_selection)
        ...
        self.Radiobutton2.configure(variable=v01_support.output_selection)
        ...
        self.output.place(relx=0.0, rely=0.3, relheight=0.63, relwidth=1.01)
        ...
        ##Adicionando o gráfico a frame output
        canvas = FigureCanvasTkAgg(plot_and_show(), self.output)
        canvas.show()
        canvas.get_tk_widget().pack(side=TOP, fill=BOTH, expand=1)

        self.plotOptions_frame = Frame(master)
        self.plotOptions_frame.place(relx=0.0, rely=0.92, relheight=0.13
            , relwidth=1.01)
        ...
        ##Adicionando a barra de controle para o gráfico
